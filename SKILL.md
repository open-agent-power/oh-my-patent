...
从「一句话」或「一堆散落的材料」走到「可提交的申请文件」：

```
用户的一句话 / 项目目录 / Word 交底书 / PPT 汇报
        │
        │  ① 脚本负责：把 Office 材料读成文本
        ├─ ingest          .docx / .pptx → Markdown（公式与图形丢失会被点名）
        ▼
   可读的文本
        │
        │  ② 你负责：想清楚「写什么」
...
**两步可以分开走**：用户已经有完整交底书时，直接从 ①→③ 开始，
不必硬走 02~05。用户只有一句话时，`01` 之后直接进 `06` 也是合法的。
不要为了「走完流程」而制造用户不需要的中间产物。
...
| 帮我扫描这个项目、项目里挖专利点 | 用户给了目录或文件 | `prompts/02-project-scan.md` → `03` | 不要把「没读到」当成「没有」——未转换的文件必须转达用户 |
| 查查有没有人做过、查新、现有技术 | 已有候选点（没有就先走 `03`） | `prompts/04-prior-art-search.md` | 不要编造 URL；不要下「具备新颖性/创造性」的结论 |
| 写技术交底书、把材料整理成交底 | 候选点已确认（没有就先走 `03`） | `prompts/05-disclosure-build.md` | 不要把交底书写成正式申请文件；不要覆盖旧稿 |
| 我这里有 Word / PPT 交底书 | —— | 先 `ingest` 转文本（见 `02`），再按需走 `03`~`05` | **不要把转换告警咽下去**——公式与图形丢失必须让用户知道 |
...
- **中间产物归位**：扫描结果、候选点、查新报告、交底书写进 `outputs/` 下按案件名
  分的子目录（如 `outputs/scan/智能水表/`），不要堆在仓库根。
- **材料丢失必须上报**：读 Office 材料时，凡遇公式退化、图形未提取、文件未转换，
  都要如实转达用户。**「我没读到」和「材料里没有」是两件完全不同的事**，
  把它们合并表述是这里最严重的失职。
- **能力自述**：用户问「你能做什么」时，如实说明可用范围是
  「读材料 → 扫描 → 挖点 → 查新 → 交底书 → 撰写 → 自检 → 出文件」；
  但**迭代修订尚未实现**（改稿请手工改了重跑 `07`），
  **CNIPA 公布公告站直查也未移植**（查新走联网检索，见 `prompts/04`）。
  不要为了显得能干而模糊这两点。

## 命令行速查

完整选项见 `prompts/06-specification-build.md` 与 `prompts/02-project-scan.md`。

```bash
# 读材料（需要可选依赖，见下）
python tools/patent.py ingest    材料目录/ -o outputs/scan/案件名/   # 批量转 Markdown
python tools/docx_to_md.py -i 交底书.docx -o 交底书.md               # 单份 Word
python tools/pptx_to_md.py -i 评审.pptx  -o 评审.md                  # 单份 PPT
...
**依赖分两层**：`build` / `lint` / `info` / `template` 只需要
`python-docx` + `PyYAML`（`requirements.txt`）；`ingest` 与两个转换脚本额外需要
`mammoth` / `markdownify` / `python-pptx`（`requirements-ingest.txt`）。
没装就只影响读 Office 材料，出文件照常。
...
- 查新（`04`）产出的是**现有技术线索与区别分析**，不是检索报告，也不是专利性结论。
  它默认走联网检索，**不覆盖按分类号的穷尽检索**。
- 读材料（`ingest`）会丢失一部分信息，且**丢失项一律被点名**（公式退化成占位、
  纯图形页无文本、旧格式文件未转换）。这些必须转达用户，不能当成「材料里没有」。
...
│   ├── 01-intake.md          立项：定类型、确认输入
│   ├── 02-project-scan.md    项目材料 → 技术事实
│   ├── 03-patent-points.md   候选专利点挖掘与融合
│   ├── 04-prior-art-search.md 查新（现有技术对照）
│   ├── 05-disclosure-build.md 技术交底书
│   ├── 06-specification-build.md 撰写申请文件 ← 核心
│   ├── 07-self-check.md      形式合规自检
│   └── iteration/            迭代修订（未实现，P4）
├── oh_my_patent/     ← 分层 Python 包（真正的逻辑）
│   ├── spec.py       版面规范常量（法规要求集中一处）
│   ├── schema.py     数据模型 + 权利要求分段/独权从权判定
│   ├── parser.py     宽容解析（Markdown / YAML / JSON）
│   ├── ingest.py     读 Office 材料（可选依赖，公式丢失会告警）
│   ├── lint.py       形式合规自检
...
└── outputs/          ← 产物（gitignore）
    ├── scan/<案件名>/       扫描结果、候选点、查新报告
    └── disclosure/<案件名>_<时间戳>.md
```
