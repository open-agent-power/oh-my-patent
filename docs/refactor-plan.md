...
| **P2** 吸收上游 | **先重新同步上游**（§7.1），再吸收项目扫描、专利点挖掘、查新、交底书 | 新增 prompts + 脚本，含测试 | **进行中**（2026-09-20，明细见 §7.3） |
...
### 7.3 P2 实际进展（2026-09-20）

| 项 | 状态 |
| --- | --- |
| 重新同步上游（§7.1 的前置） | ✅ `docs/upstream-sync-c4ae70a.md` |
| 材料读取层 | ✅ `oh_my_patent/ingest.py` + `tools/docx_to_md.py` / `pptx_to_md.py`，33 项新测试 |
| `prompts/02`~`05` | ✅ 由「未实现」改为**可执行流程** |
| `tools/cnipa.py`（CNIPA 公布站直查） | ❌ 未做。`04` 只走联网检索，并在文件里如实写明 |
| 全链路验收（扫描 → 挖点 → 交底书） | ⚠ **走到 `03` 按纪律停住**，见下 |

**关于最后一项**：验收用的是 `D:/tmp/p2-accept/智能水表项目`（含 `docx`/`pptx`/
`.doc`/`.pdf`/`node_modules`/`.git`/`~$` 锁文件的杂乱项目）。`02` 与 `03` 的产物
已落在 `outputs/scan/智能水表/`（该目录 gitignore，真实案件稿件不进公开仓库）。

走到 `03` 时暴露出三处信息缺口，**按 `03` 自己的纪律不得带着窟窿进 `05`**，
因此停在这里等用户回答：

1. 滞回带宽度公式两处占位疑似冲突（`W=(H-L)/2` 与 `W=H/2`，取值差一个数量级）；
2. H/L 的取值依据缺失（评审备注点名要求给出，材料里没有）；
3. 评审第 4 页「系统架构」为纯图形，未读到。

**这恰恰是验收想要的结论**：流程能拦住「带着窟窿往下做」，而不是一路生成到
`06` 才爆出来。

### 7.4 P2 验收发现并修掉的一个缺陷

`markdown_header()` 原先只写 `warning` 级提示，而 `SLIDE_TITLE_ONLY` /
`SLIDE_EMPTY` 是 `info` 级。后果：

- `prompts/02` 明确要求「先读 `.md` 顶部注记，再读正文」，并把这两个码列在
  必须逐条处理的告警表里；
- 但注记里**根本没有它们**——验收时评审 PPT 的第 4 页只剩「系统架构」四个字，
  Agent 读到的是一页空内容，绝不会想到它原本是整张架构图。

而 `02` 全文最强调的一条正是「**我没读到** 和 **材料里没有** 是两件不同的事」。
同一个过滤逻辑在 `markdown_header` 与 `cli` 各写了一遍、两处都只取 `warnings`，
所以同一个提示在两个出口同时消失。

修法：把分组收敛进 `IngestResult.warnings` / `.infos` 两个属性，两个出口都改为
按级别分组呈现**全部**提示；新增 3 项测试守住「产生 ≠ 送达」这条链路。
